...
window.addEvent('domready', function() { // wait for the content
  ...
  var up = new FancyUpload2($('demo-status'), $('demo-list'), { // options object
    ...
    url: $('products_create').action + '?ul=1',
    ...
    onFileRemove: function(file) {
        ...
        'onSuccess' : function(responseJSON) {
          var photo_preview_element = document.getElementById("photo_preview-wrapper");
          if ($('photo_preview-element').getChildren().length == 1) {
            photo_preview_element.style.display = "none";
            $('demo-list').style.display = 'none';
          }
          $('demo-browse').style.display = "inline-block";
          return false;
        }
      });
      ...
      if ($('photo_preview_'+file_id)) $('photo_preview_'+file_id).dispose();
      else $('photo_preview').dispose();
    },
    ...
    onFileSuccess: function(file, response) {
      ...
      if (json.get('status') == '1') {
        ...
        var photo_preview_element = document.getElementById("photo_preview-wrapper");
        photo_preview_element.style.display = "block";

        var newElement = new Element('input', {
          id: 'photo_preview_' + json.get('photo_id'),
          type: 'image',
          src: json.get('photo_url'),
          style: 'max-width: 200px; margin-left: 20px;',
        });
        newElement.inject($('photo_preview-element'), 'bottom');
        if ($('photo_preview-element').getChildren().length == 4) {
          $$('.buttonlink.icon_photos_new ').hide()
        }
      } else {
...
<input type="hidden" name="photo" id="fancyuploadfileids" value ="" />
