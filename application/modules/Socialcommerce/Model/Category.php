...
class Socialcommerce_Model_Category extends Socialcommerce_Model_Node
{
    ...
    public function setPhoto($photo)
    {
        if ($photo instanceof Zend_Form_Element_File)
        {
            ...
            if (is_array($photo) && !empty($photo['tmp_name']))
            {
                ...
                {
                    throw new Socialcommerce_Model_Exception('Invalid argument passed to setPhoto: ' . print_r($photo, 1));
                }

        $name = basename($file);
        $path = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary';
        $params = array(
            'parent_type' => 'socialcommerce',
    ...
    public function getCustomFieldsList() {
        $mapData = Engine_Api::_()->getApi('core', 'fields')->getFieldsMaps('socialcommerce_listing');
    ...
    public function getNumOfListings() {
        $table = Engine_Api::_()->getDbTable('listing', 'socialcommerce');
