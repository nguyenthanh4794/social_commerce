 ...
 * Time: 9:20 PM
 */
class Socialcommerce_Model_DbTable_Nodes extends Engine_Db_Table
{
    protected $_rowClass = 'Socialcommerce_Model_Node';
    ...
    /**
     * @return Socialcommerce_Model_Node
     */
     ...
     * @param  $parent_id
     * @return Socialcommerce_Model_Node|NULL
     */
     ...
     * @param   Socialcommerce_Model_Node  $node
     * @param   Array                $data
     * @return  Socialcommerce_Model_Node
     * @throw Exception
     */
    public function insertAfter(Socialcommerce_Model_Node $node, $data) {
        if($node -> pleft == 1) {
        ...
        try {
            ...
            $this -> updateTree();
            $optionId = Engine_Api::_()->getApi('core', 'Socialcommerce')->typeCreate($newNode->title);
     ...
     * @throw Exception
     */
    public function insertBefore(Socialcommerce_Model_Node$node, $data) {
        if($node -> pleft == 1) {
            throw new Exception("can not insert after root");
        }
     ...
     * @throw Exception
     */
    public function appendChild(Socialcommerce_Model_Node $node, $data) {
     ...
     * @throw Exception
     */
    public function prependChild(Socialcommerce_Model_Node$node, $data) {
        $newNode = $this -> fetchNew();
        $newNode -> setFromArray($data);
        $anchor = $node -> pleft;
        $newNode -> pleft = $anchor + 1;
     ...
     * @throw Exception
     */
    public function addChild(Socialcommerce_Model_Node $node, $data) {
        if(!is_object($node)) {
            $node = $this -> getRoot();
        }
    ...
    public function getDescendent($node, $include_own =true) {
        if(!$node instanceof Socialcommerce_Model_Node) {
